...
# 生成序列的相对位姿 it为间隔帧数，0为连续帧 返回(x,y,z,r_x,r_y,r_z)
def generate_xyz_euler_relative(it):
    dir_data = 'xyz-euler-relative-interval{:d}/'.format(it)
    if not os.path.exists(dir_data):
        os.mkdir(dir_data)

    for i in tqdm(range(11)):  # 11个序列
        t_12d = np.loadtxt('./poses/{:02d}.txt'.format(i))
        ret = np.zeros((len(t_12d) - it - 1, 6))  # 保存样本数 * 6D的位姿数据
        for j in range(len(t_12d) - it - 1):
            t1 = mat(np.row_stack((np.reshape(t_12d[j, :], (3, 4)), [0, 0, 0, 1])))  # t时刻位姿
            t2 = mat(np.row_stack((np.reshape(t_12d[j + it + 1, :], (3, 4)), [0, 0, 0, 1])))  # t+it时刻数据
            t12 = t1.I * t2  # T1的逆左乘T2，得到T1到T2的位姿变换矩阵，即相对位姿变换T12
            ret[j, 0: 3] = [t12[0, 3], t12[1, 3], t12[2, 3]]

            # rotation matrix to ypr 将旋转矩阵转换成偏航角，俯仰角和翻滚角
...
def validate_xyz_euler_relative(seq):
    pose_gt = np.loadtxt('./poses/{:02d}.txt'.format(seq))
...
def generate_xyz_euler_relative_reverse(it):
    ...
    for i in tqdm(range(11)):
        ...
        for j in range(len(gt) - it - 1):
            ...
            t12 = t2.I * t1  # 这里t12改成t21其实更加便于理解
            ret[j, 0: 3] = [t12[0, 3], t12[1, 3], t12[2, 3]]

...
def inverse_warp2(img, depth, ref_depth, pose, intrinsics, padding_mode='zeros'):
    ...
    projected_img = F.grid_sample(img.cuda(), src_pixel_coords, padding_mode=padding_mode, align_corners=False)

    valid_points = src_pixel_coords.abs().max(dim=-1)[0] <= 1
    valid_mask = valid_points.unsqueeze(1).float()

    projected_depth = F.grid_sample(ref_depth.cuda(), src_pixel_coords, padding_mode=padding_mode, align_corners=False)
